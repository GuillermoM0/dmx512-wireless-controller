...
/* Private typedef -----------------------------------------------------------*/
typedef struct{
  uint16_t  CustomDevice_ServiceHdle;                    /**< DEVICE_SERVICE handle */
  uint16_t  CustomV_ChHdle;                  /**< VALUE_AND_CHANEL handle */
  uint16_t  CustomFullHdle;                  /**< FULL_BUFFER handle */
...
/* Private variables ---------------------------------------------------------*/
uint16_t SizeV_Ch = 512;
uint16_t SizeFull = 512;
...
#define COPY_DEVICE_SERVICE_UUID(uuid_struct)          COPY_UUID_128(uuid_struct,0xb7,0x4f,0x00,0x00,0x8d,0x4a,0x4c,0x78,0xb6,0x01,0x7b,0x2b,0xa1,0x10,0x22,0xe3)
#define COPY_VALUE_AND_CHANEL_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0xb7,0x4f,0x00,0x01,0x8d,0x4a,0x4c,0x78,0xb6,0x01,0x7b,0x2b,0xa1,0x10,0x22,0xe3)
#define COPY_FULL_BUFFER_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0xb7,0x4f,0x00,0x02,0x8d,0x4a,0x4c,0x78,0xb6,0x01,0x7b,0x2b,0xa1,0x10,0x22,0xe3)
...
//            Notification.Custom_Evt_Opcode = CUSTOM_STM_V_CH_WRITE_NO_RESP_EVT;
...
void SVCCTL_InitCustomSvc(void)
{
   ...
   * service_max_attribute_record = 1 for DEVICE_SERVICE +
   *                                2 for VALUE_AND_CHANEL +
   *                                2 for FULL_BUFFER +
   *                              = 5
   *
...
tBleStatus Custom_STM_App_Update_Char(Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_FULL:
      ret = aci_gatt_update_char_value(CustomContext.CustomDevice_ServiceHdle,
                                       CustomContext.CustomFullHdle,
                                       0, /* charValOffset */
                                       SizeFull, /* charValueLen */
                                       (uint8_t *)  pPayload);
      if (ret != BLE_STATUS_SUCCESS)
      {
...
tBleStatus Custom_STM_App_Update_Char_Ext(uint16_t Connection_Handle, Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_V_CH:
      ...
      ret = Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomDevice_ServiceHdle, CustomContext.CustomV_ChHdle, SizeV_Ch, pPayload);
    ...
    case CUSTOM_STM_FULL:
      ...
      ret = Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomDevice_ServiceHdle, CustomContext.CustomFullHdle, SizeFull, pPayload);
